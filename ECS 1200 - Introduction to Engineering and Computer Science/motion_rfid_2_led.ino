...
int redFlashing;
signed long redFlashingTimer;
signed long rocketLauncherTimer;
#define flashingInterval 75
#define rocketInterval 3400
void setup() {                
...
void loop()
{
	...
	uchar status;
        ...
        if(currentMillis - redFlashingTimer > flashingInterval)
        {
          redFlashingTimer = currentMillis;
          if(redFlashing == LOW)
            redFlashing = HIGH;
          else
            redFlashing = LOW;
        ...
        if(rocketLauncherTimer != -1 && currentMillis > rocketLauncherTimer)
        {
          digitalWrite(rocketLauncher,LOW);
           rocketLauncherTimer = -1; 
        }
        ...
        if(digitalRead(motionSensor))
        {
          ...
          Serial.println("quiick shoot!");
          digitalWrite(redLED,redFlashing);
          digitalWrite(rocketLauncher,HIGH);
          if(rocketLauncherTimer == -1)
            rocketLauncherTimer = millis() + rocketInterval;
        }
		...
		if (status == MI_OK)
		{
			...
			Serial.print(serNum[4],BIN);
                        ...
                        */
                        ...
                        if(serNum[0] == 11)
                        {
                           //white card was scanned
                          Serial.println("ACCEPTED"); 
                          
                          //pause the code to let someone in
                          digitalWrite(redLED,LOW);
                          digitalWrite(greenLED, HIGH);
                          digitalWrite(rocketLauncher, LOW);
                          Serial.println("Pausing code.");
                          delay(10000);
